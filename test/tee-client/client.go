...
func NewTEEClient(addr string) (*TEEClinet, error) {
	tc := TEEClinet{}
	...
	fmt.Println("connected:", string(msg))
...
func (w *TEEClinet) Invoke(url string, data []byte) ([]byte, error) {
	...
	if resp.Code != 0 {
		return nil, fmt.Errorf("code: %d, msg: %s", resp.Code, resp.Data)
	}
